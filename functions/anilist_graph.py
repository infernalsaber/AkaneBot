...
class AnimeNode:
    def __init__(self, id: int, title: str, date_info: Dict, duration: int = 0, episodes: int = 0):
        self.id = id
        self.title = title
    ...
    def __str__(self):
        ...
        return f"{self.title}{watch_time}"
...
def format_watch_order(anime_id: int, include_side_stories: bool = False) -> str:
    ...
    series_name = find_series_name([anime.title.lower() for anime in order])
...
def find_series_name(titles, threshold=0.6):
    ...
    candidates = sorted(all_substrings.items(), 
...
def get_complete_series(session, anime_id: int, visited_ids: Set[int] = None) -> List[AnimeNode]:
    ...
    if media['relations']:
        for edge in media['relations']['edges']:
            node = edge['node']
            if node['type'] == 'ANIME' and edge['relationType'] in ['PREQUEL', 'SEQUEL', 'SIDE_STORY']:  # Only process anime entries
